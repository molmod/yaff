...
with h5.File('output.h5', mode='w') as f:
    ...
    thermo = NHCThermostat(temp=300)
    #thermo = LangevinThermostat(temp=300)
    verlet = VerletIntegrator(ff, 1*femtosecond, hooks=[hdf5_writer, xyz, sl, thermo], temp0=300)
    verlet.run(500)

    # 4) analyze md
    plot_temp_dist(f, ndof=verlet.ndof)
    plot_energies(f)
